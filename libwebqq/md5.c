...
#define S44 21
 ...
 void hash_get_str(const char* digest, char* outhex)
{
	char buffer[16] = { 0 };
	int i = 0;
	...
	for (; i < 20; i++)
	{
		sprintf(buffer, "%02X", (unsigned char)digest[i]);
		strcat(outhex, buffer);
	}
}
...
char* lutil_md5_digest(const unsigned char *data, unsigned int len, char *buf)
{
	...
	hash_get_str(ctx.digest, buf);

	return buf;
}

void lutil_md5_data (const unsigned char *data, unsigned int len, char *buf)
{
	MD5_CTX ctx;

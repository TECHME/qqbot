...
struct irc_conn {
	...
	int fd;
	size_t timer;
	std::map<int,int >	buddies;

	bool ison_outstanding;
	std::list<int> buddies_outstanding;
...
bool irc_blist_timeout(struct irc_conn *irc);
bool irc_who_channel_timeout(struct irc_conn *irc);
void irc_buddy_query(struct irc_conn *irc);

char *irc_escape_privmsg(const char *text, size_t length);
...
bool irc_ischannel(const char *string);
...
// void *irc_dccsend_new_xfer(PurpleConnection *gc, const char *who);
